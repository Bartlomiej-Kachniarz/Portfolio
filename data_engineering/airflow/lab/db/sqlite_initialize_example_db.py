...
def create_table_in_db():
    db_path = rf"{path_to_db}/example_db.db"

    conn = create_conn(db_path)

    if conn is not None:
        with open(f"{path_to_db}/sql/create_projects_table.sql", "r") as f:
            sql_query = f.read()
            create_table(conn, sql_query)

        with open(f"{path_to_db}/sql/create_tasks_table.sql", "r") as f:
            sql_query = f.read()
            create_table(conn, sql_query)
    else:
